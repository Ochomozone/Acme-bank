...
app.post("/download", function (request, response) {
  if (request.session.loggedin) {
    ...
    const root_directory = path.join(process.cwd());
    rootDir = "history_files\\"
    const filePath = root_directory + "/history_files/" + file_name;
...
app.post("/public_forum",  body('comment').notEmpty().withMessage('Cannot be empty').escape().isLength( {max: 250}).withMessage("Must be less than 250 characters"), function (request, response) {
